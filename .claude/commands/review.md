...
レビュー時に以下のインストール済みスキルの知識を活用すること：

- **ruby-on-rails-best-practices** — Rails変更のレビュー時に参照。Basecamp流のRails規約（モデル設計、コントローラー設計、ジョブ設計など）に基づいて指摘する
- **vercel-react-best-practices** — Next.js/React変更のレビュー時に参照。Vercel公式のパフォーマンス最適化パターンに基づいて指摘する
- **code-review-excellence** — 全レビューの品質基準として参照。指摘の優先度分類（blocking/important/nit）やフィードバックの書き方に活用する

各サブエージェントは、該当するスキルファイルを読み込んでからレビューを行うこと。

### レビュー観点

#### 1. コード品質（subagent: sonnet）
...
#### 2. セキュリティ（subagent: sonnet）
- OWASP Top 10（SQLインジェクション、XSS、CSRF等）
- 認証・認可の適切な実装
- 機密情報の漏洩リスク（.env、credentials等）
- 入力バリデーションの漏れ

#### 3. Rails/Next.js ベストプラクティス（subagent: sonnet）
